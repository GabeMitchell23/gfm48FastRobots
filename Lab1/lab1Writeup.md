...
The code is run in the video below. At each octave, an A#, an A, and an A♭ are played to show if the code can distinguish an A from tones directly preceeding and succeeding it. The code performs well at high frequencies. However, it confuses an A# for an A at lower tones.
